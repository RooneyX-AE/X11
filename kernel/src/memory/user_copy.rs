...
use super::{validate_slice, Page4K, PageAccess, PageTableMapper, UserRangeError, PAGE_SIZE_4K};
...
pub trait UserCopyBackend {
    /// Copies bytes from an already validated userspace address into `dst`.
    /// The backend owns the architecture-specific translation and access
    /// mechanism. Implementations must not read outside `src..src+dst.len()`.
    fn copy_from_user(&self, src: u64, dst: &mut [u8]) -> Result<(), UserReadError>;
}

pub fn validate_readable_range<M: PageTableMapper>(
    mapper: &M,
    slice: UserSlice,
    ...
    loop {
        ...
        match mapper.page_access(page.start_address()) {
            ...
            PageAccess { mapped: true, user: false, .. } => {
                return Err(UserReadError::NotUserAccessible)
            }
            PageAccess { mapped: true, user: true, readable: false, .. } => {
                return Err(UserReadError::NotReadable)
            }
...
pub fn copy_from_user<M: PageTableMapper, B: UserCopyBackend>(
    mapper: &M,
    backend: &B,
    slice: UserSlice,
    dst: &mut [u8],
...
mod tests {
    ...
    impl PageTableMapper for FakeMapper {
        ...
        fn address_space(&self) -> super::super::VirtRange {
            super::super::VirtRange::new(USER_SPACE_START, KERNEL_SPACE_START).unwrap()
        }
    }
    ...
    fn rejects_kernel_only_mapping() {
        ...
        assert_eq!(
            validate_readable_range(&mapper, UserSlice { ptr: USER_SPACE_START, len: 1 }),
            Err(UserReadError::NotUserAccessible)
        );
    }
    ...
    fn copy_requires_destination_length_match() {
        ...
        let mut dst = [0u8; 4];
        assert!(matches!(
            copy_from_user(&mapper, &backend, UserSlice { ptr: USER_SPACE_START, len: 8 }, &mut dst),
            Err(UserReadError::InvalidRange(_))
        ));
    }
    ...
    fn copy_rejects_unmapped_before_backend() {
        ...
        let mut dst = [0u8; 4];
        assert_eq!(
            copy_from_user(&mapper, &backend, UserSlice { ptr: USER_SPACE_START, len: 4 }, &mut dst),
            Err(UserReadError::Unmapped)
        );
    }
}