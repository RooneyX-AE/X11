...
impl VirtRange {
    pub const fn new(start: u64, end: u64) -> Option<Self> { if start <= end { Some(Self { start, end }) } else { None } }
...
mod tests {
    ...
    #[test] fn rejects_inverted_range() { assert_eq!(VirtRange::new(2, 1), None); }
    #[test] fn recognizes_user_range() { let range = VirtRange::new(USER_SPACE_START, USER_SPACE_START + 0x4000).unwrap(); assert!(range.is_user()); assert!(!range.is_kernel()); }
    #[test] fn recognizes_kernel_range() { let range = VirtRange::new(KERNEL_SPACE_START, KERNEL_SPACE_START + 0x4000).unwrap(); assert!(range.is_kernel()); assert!(!range.is_user()); }
}
