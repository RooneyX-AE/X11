...
extern "C" fn preemption_task_a() -> ! { loop { core::hint::spin_loop(); } }
extern "C" fn preemption_task_b() -> ! { loop { core::hint::spin_loop(); } }
...
fn kernel_main(boot_info: &'static mut BootInfo) -> ! {
    ...
    let userspace_image = match ramdisk::bytes(boot_info) {
        Ok(bytes) => {
            ...
            serial::write_str("\r\n");
            let address_space = process::AddressSpaceSpec::new(process::AddressSpaceId::new(1).expect("first address-space id must be non-zero"));
            match process::build_process_image(bytes, address_space) {
                ...
                Err(_) => { serial::write_str("X11-OS: userspace process image preparation failed\r\n"); None }
            }
        }
    ...
    let tsc_clock = match arch::x86_64::tsc::TscClocksource::try_new() {
        Ok(clock) => { serial::write_str("X11-OS: invariant TSC clocksource available\r\n"); serial::write_str("X11-OS: TSC frequency Hz = "); serial::write_usize(clock.frequency().hz() as usize); serial::write_str("\r\n"); Some(clock) }
        Err(_) => { serial::write_str("X11-OS: invariant TSC clocksource unavailable\r\n"); None }
    };

    let apic = arch::x86_64::apic::ApicCapabilities::detect();
    serial::write_str("X11-OS: local APIC = "); serial::write_str(if apic.apic { "supported\r\n" } else { "unsupported\r\n" });
    serial::write_str("X11-OS: x2APIC = "); serial::write_str(if apic.x2apic { "supported\r\n" } else { "unsupported\r\n" });

    let memory = memory::summarize_boot_map(&boot_info.memory_regions);
    serial::write_str("X11-OS: usable memory bytes = "); serial::write_usize(memory.usable_bytes() as usize); serial::write_str("\r\n");
    serial::write_str("X11-OS: reserved memory bytes = "); serial::write_usize(memory.reserved_bytes() as usize); serial::write_str("\r\n");
    serial::write_str("X11-OS: malformed memory regions = "); serial::write_usize(memory.malformed_regions() as usize); serial::write_str("\r\n");

    let physical_mapping = memory::PhysicalMemoryMapping::from_boot_info(boot_info);
    if let Some(mapping) = physical_mapping {
        let _ = mapping.install_global();
    }
    ...
    if let (Some(mapping), Some(rsdp_address)) = (physical_mapping, Option::<u64>::from(boot_info.rsdp_addr)) {
        match unsafe { arch::x86_64::acpi::discover(rsdp_address, mapping) } {
            Ok(topology) => {
                ...
                serial::write_str("X11-OS: local APIC records = "); serial::write_usize(topology.local_apic_count); serial::write_str("\r\n");
                serial::write_str("X11-OS: local x2APIC records = "); serial::write_usize(topology.local_x2apic_count); serial::write_str("\r\n");
                serial::write_str("X11-OS: I/O APIC records = "); serial::write_usize(topology.io_apic_count); serial::write_str("\r\n");
                if topology.io_apic_count > 0 {
                    ...
                    if let Some(mode) = apic.preferred_mode() {
                        if unsafe { arch::x86_64::apic::enable_preferred_mode(apic) } == Some(mode) {
                            serial::write_str("X11-OS: local APIC mode = "); serial::write_str(match mode { arch::x86_64::apic::ApicMode::XApic => "xAPIC\r\n", arch::x86_64::apic::ApicMode::X2Apic => "x2APIC\r\n" });
    ...
    } else { serial::write_str("X11-OS: ACPI RSDP unavailable\r\n"); }
    ...
    if let (true, Some(mapping), Some(tsc)) = (apic_ready, physical_mapping, tsc_clock.as_ref()) {
        match unsafe { arch::x86_64::lapic_timer::LapicTimer::new(apic.preferred_mode().unwrap(), Some(mapping)) } {
            Ok(mut timer_device) => match timer_device.calibrate(tsc) {
                Ok(frequency) => { serial::write_str("X11-OS: LAPIC timer calibrated Hz = "); serial::write_usize(frequency.hz() as usize); serial::write_str("\r\n"); let _ = timer_device.set_periodic(timer::TimerInterval::new(10_000_000).unwrap()); }
            ...
            if let (Some(physical_range), Some(size)) = (frames.physical_range(), frames.byte_len()) {
                if let Some(virtual_range) = mapping.translate_range(physical_range) {
                    if let Some(region) = heap::HeapRegion::new(virtual_range.start(), heap::INITIAL_HEAP_SIZE) {
                        if size == heap::INITIAL_HEAP_SIZE && virtual_range.len() == size as u64 && heap::GLOBAL.init(region).is_ok() {
    ...
    let prepared_user_launch = match (userspace_image, physical_mapping) {
        (Some((bytes, image)), Some(mapping)) => match process::ElfImage::parse(bytes) {
            ...
            Err(_) => { serial::write_str("X11-OS: userspace ELF re-parse failed\r\n"); None }
        },
        _ => None,
    };
