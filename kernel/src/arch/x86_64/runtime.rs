...
//! itself must remain at a stable address while any spawned task can yield.
...
impl KernelRuntime {
    ...
    pub fn spawn(
        ...
        self.manager
            .spawn(priority, entry)
            .map_err(RuntimeError::Task)
    }

    /// Selects the next runnable task without performing the CPU switch.
    ...
    /// Validates that a runnable task has a concrete execution context.
