...
impl InterruptReturnFrame {
    ...
    /// Returns the stack pointer that will be active after `iretq` consumes the
    /// return frame. For same-CPL kernel returns, the CPU did not save RSP, so
    /// it is derived from the current frame address plus the three words that
    /// `iretq` pops in 64-bit mode.
    pub fn resume_rsp(self) -> Option<u64> {
        let raw = self.raw as usize;
        let bytes = if unsafe { self.cs() } & 3 == 0 { 24 } else { 40 };
        raw.checked_add(bytes).map(|value| value as u64)
    }
