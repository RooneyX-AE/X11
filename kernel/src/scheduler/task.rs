...
/// Mutable lifecycle record owned exclusively by the scheduler.
///
/// A task control block is intentionally not `Copy`: copying it would duplicate
/// the scheduler's logical ownership of the same execution handle and make it
/// possible to manufacture two local representations of one task instance.
#[derive(Debug)]
...
mod tests {
    ...
    #[test]
    fn task_state_machine_rejects_terminal_reentry() {
        let id = TaskId::new(3, 7);
        let mut task = TaskControlBlock::new(id, Priority::DEFAULT);
        assert!(task.transition(TaskState::Ready));
        assert!(task.transition(TaskState::Running));
        assert!(task.transition(TaskState::Exited));
        assert!(!task.transition(TaskState::Ready));
        assert!(!task.transition(TaskState::Running));
